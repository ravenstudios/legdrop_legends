...
class MovementHandler():
    ...
    def key_handler(self, cam_offset):
        ...
        world_bottom_edge = WORLD_HEIGHT - GAME_HEIGHT - self.player.speed
        ...
        if keys[pygame.K_UP]:
            ...
            if self.can_move_x_left:
                self.player.x -= self.player.speed
            if self.player.x == 0 or self.player.x >= world_right_edge and self.player.rect.center[0] >= half_screen_w:
                self.can_move_x_left = False
                self.player.rect.x -= self.player.speed
            else:
